...
import { updateFdAmount, updateFdCreditLimit, setUserInfoData } from "redux/actions";

export default function Secure() {
    const trackingId = useSelector(state => state.trackingId.trackingId);
    const products = useSelector(state => state.product.product);   
    const cardSubType = products.cardSubType;
    const isDream = trackingId != null && cardSubType == 'DREAM' ? true : false;
    ...
    function handleChange(value){
       ...
       dispatch(updateFdAmount(inputV));
    }
    const handleClick = e => {  
        disableOnceClick();
        UpdatePageDetail();
        dispatch(updateFdCreditLimit(fixeddeposit));
    ...
    const UpdatePageDetail = async (event) => {
        let data = { "trackingId": trackingId, "pageName": "FD_DETAILS_ENTERED" };
        try {
            ...
            case 200:
              if (result.statusCode === 200) {
                if (result.status === "SUCCESS") {
                  dispatch(setUserInfoData(result.data.message));
           ...
           <Layout title='SCC_FD_Details_Enter-PreLogin - kotak 811'>
                ...
                <div className={fixed.fixedpage}>
                    ...
                    <div className={fixed.check}>                        
                        ...
                        <span>I irrevocably agree and accept to have read and understood all the <TermsandCondition /> applicable to <CreditPdf /> , and <Lien /> on your fixed deposit and agree to be bound by and to be able to abide by the same.</span>
                    </div>
            ...
            </div>  : <p>Restricted Access. Redirecting to Home Page</p> }
        </>
    );
}